...
def _memory_json_suffix(nested: bool = False) -> str:
    ...
    return (
        ...
        "- \"change_event_types\": one list of event tags for each segment, with the same count as summaries.\n"
        "Transitions must be between image 1 and the final image; never use 0.\n"
        "summary[0] covers images before the first transition, and the final summary covers through the final image.\n"
        "For example, transitions [6] requires two summaries: one for images 0-5 and one for images 6-final.\n"
        "Use sampled image indices only, not timestamps or original video frame numbers.\n"
    )
