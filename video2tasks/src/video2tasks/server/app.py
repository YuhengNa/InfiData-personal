...
from ..validation import validate_memory_annotation, validate_memory_coverage
...
def create_app(config: Config) -> FastAPI:
    ...
    def submit_result(res: SubmitModel) -> Dict[str, str]:
        ...
        if res.vlm_json and "memory" in config.annotation.targets:
            validation_error = validate_memory_annotation(
                res.vlm_json,
                len(res.meta.get("frame_ids") or []),
                nested=len(config.annotation.targets) > 1,
            )

        # Empty or invalid result: trigger retry
        if not res.vlm_json or validation_error:
            if job_info:
                with queue_lock:
                    ...
                    if retry_counts[tid] <= config.server.max_retries_per_job:
                        ...
                        reason = validation_error or "empty result"
                        print(
                            f"[Warn] Task {tid} invalid ({reason}), "
                            f"re-queueing (attempt {retry_counts[tid]})"
                        )
                    else:
                    ...
                    try:
                        ...
                        if len(by_wid) >= len(windows):
                            ...
                            if "memory" in config.annotation.targets:
                                ...
                                coverage_error = validate_memory_coverage(memory_records, nframes)
                                if coverage_error:
                                    print(
                                        f"[Warn] {ctx.subset}/{sid} invalid memory coverage "
                                        f"({coverage_error}); re-annotating"
                                    )
                                    Path(w_path).unlink(missing_ok=True)
                                    sample_status[sid] = 0
                                    continue
