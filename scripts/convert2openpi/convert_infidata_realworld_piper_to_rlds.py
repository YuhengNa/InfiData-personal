"""Convert normalized InfiData realworld_piper episodes to RLDS/TFDS.

The normalized InfiData root currently has two fixed schema variants:

* piper_s14_a14_fps30_c4_ee_pose: 4 RGB cameras plus observation/action EE pose.
* piper_s14_a14_fps15_c3_no_ee_pose: 3 RGB cameras without EE pose.

This converter builds one RLDS dataset per schema variant so fields stay
truthful and fixed-shape. It intentionally preserves realworld_piper semantics
instead of reshaping into DROID/RoboMIND/OpenPI historical schemas.
...
Example smoke test:
    python scripts/convert2openpi/convert_infidata_realworld_piper_to_rlds.py \
        ...
        --data-dir /mnt/workspace/tmp/realworld_piper_rlds_one \
        --max-episodes 1 \
        --overwrite
"""
...
def _slugify(value: str) -> str:
    ...
    return value or "realworld_piper_schema"
    ...
    for (schema_key, cameras), (episode_count, frame_count) in counts.items():
        ...
        variants.append(
            SchemaVariant(
                ...
                has_ee_pose=bool(config.get("has_ee_pose", False)),
            )
        )
...
def _print_schema_variants(variants: list[SchemaVariant]) -> None:
    print("Available realworld_piper RLDS variants:")
    for variant in variants:
        ...
        print(
            ...
            f"frames={variant.frame_count:,} | state_dim={variant.state_dim} | "
            f"action_dim={variant.action_dim} | fps={variant.fps} | has_ee_pose={variant.has_ee_pose} | "
            f"slug={variant.slug}"
        )
...
def _real_scalar_columns(infidata_root: Path, episode: InfiEpisode) -> tuple[str, ...]:
    rows = pd.read_parquet(infidata_root / episode.parquet_path, columns=None)
    return tuple(column for column in REAL_SCALAR_COLUMNS if column in rows.columns)
...
class RealworldPiperInfidata(tfds.core.GeneratorBasedBuilder):
    VERSION = tfds.core.Version("1.0.0")
    RELEASE_NOTES = {"1.0.0": "Semantic-preserving InfiData realworld_piper RLDS split by schema."}
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._has_ee_pose = has_ee_pose
        self._real_scalar_columns = real_scalar_columns
        self._robot_config = robot_config
        self._robots_json = robots_json
        self._skip_log_path = Path(skip_log_path)
        self._state_action_schema = robot_config.get("state_action_schema", {})
    ...
    def _info(self) -> tfds.core.DatasetInfo:
        ...
        step_features: dict[str, Any] = {
            ...
            "quality_annotation_status": tfds.features.Text(),
        ...
        if self._has_ee_pose:
            observation_features["ee_pose"] = tfds.features.Tensor(shape=(12,), dtype=np.float32)
            step_features["action_ee_pose"] = tfds.features.Tensor(shape=(12,), dtype=np.float32)
        for column in self._real_scalar_columns:
            dtype = np.int64 if column.endswith("_ns") else np.float32
            step_features[column] = dtype

        return tfds.core.DatasetInfo(
            builder=self,
            description=(
                "InfiData realworld_piper converted to semantic-preserving RLDS. "
                "Each generated dataset contains one fixed piper schema variant."
            ),
            features=tfds.features.FeaturesDict(
                {
                    ...
                    "episode_metadata": {
                        ...
                        "ee_pose_schema_json": tfds.features.Text(),
    ...
    def _episode_metadata(self, episode: InfiEpisode, rows: pd.DataFrame) -> dict[str, Any]:
        first = rows.iloc[0]
        state_action_schema = self._state_action_schema
        return {
            ...
            "tasks_json": _json_dumps(self._metadata_value(episode, "tasks", [])),
            ...
            "has_ee_pose": bool(self._metadata_value(episode, "has_ee_pose", self._has_ee_pose)),
            ...
            "state_action_schema_json": _json_dumps(state_action_schema),
            "state_representation": _as_text(state_action_schema.get("state_representation"), ""),
            "action_representation": _as_text(state_action_schema.get("action_representation"), ""),
            "action_is_delta": bool(state_action_schema.get("action_is_delta", False)),
            "ee_pose_schema_json": _json_dumps(
                {
                    "feature": self._robot_config.get("ee_pose_feature", {}),
                    "coordinate_frame": state_action_schema.get("ee_pose_coordinate_frame", ""),
                    "note": state_action_schema.get("ee_pose_note", ""),
    ...
    def _generate_examples(self, split_name: str, episodes: list[InfiEpisode]):
        ...
        progress = tqdm.tqdm(
            total=len(episodes),
            desc=f"Converting realworld_piper {self._schema_key} {split_name} ({total_frames:,} frames)",
            unit="ep",
            dynamic_ncols=True,
        )
        ...
        try:
            for episode in episodes:
                ...
                try:
                    ...
                    print(
                        f"[skip] realworld_piper {split_name} episode={episode.episode_index}: {exc}",
                        flush=True,
                    )
                    progress.update(1)
                    continue
                ...
                for i, row in rows.iterrows():
                    ...
                    if self._has_ee_pose:
                        observation["ee_pose"] = _as_float_array(row["observation.ee_pose"], (12,))
                        step["action_ee_pose"] = _as_float_array(row["action.ee_pose"], (12,))
                    for column in TEXT_COLUMNS:
                        step[column] = _as_text(row.get(column), "")
                    for column in self._real_scalar_columns:
                        if column.endswith("_ns"):
                            step[column] = int(row.get(column, 0))
                        else:
                            step[column] = np.float32(row.get(column, 0.0))
                    steps.append(step)
...
def _build_one_variant(
    *,
    ...
    image_shapes = _infer_image_shapes(infidata_root, selected_episodes[0], variant.cameras)
    real_scalar_columns = _real_scalar_columns(infidata_root, selected_episodes[0])
    total_frames = sum(episode.num_frames for episode in selected_episodes)
    print(
        f"Selected {len(selected_episodes):,} realworld_piper episodes, {total_frames:,} frames, "
        ...
        f"has_ee_pose={variant.has_ee_pose}."
    )
    print(
    ...
    print(f"Real scalar fields: {', '.join(real_scalar_columns) if real_scalar_columns else '<none>'}")
    print(f"Image shapes: {image_shapes}")

    if overwrite:
        dataset_dir = data_dir / "realworld_piper_infidata"
    ...
    builder = RealworldPiperInfidata(
        ...
        has_ee_pose=variant.has_ee_pose,
        real_scalar_columns=real_scalar_columns,
...
def main() -> None:
    ...
    parser.add_argument("--infidata-root", type=Path, default=Path("/mnt/workspace/InfiData/realworld_piper_infidata"))
    parser.add_argument("--data-dir", type=Path, default=Path("/mnt/workspace/RLDS/realworld_piper"))
    ...
    parser.add_argument("--all-schemas", action="store_true", help="Build every schema variant.")
    ...
    if not matching:
        raise ValueError(f"No realworld_piper variant matched schema_key={args.schema_key!r}, camera_set={camera_set!r}.")
    if len(matching) > 1:
